 ...
 */
internal class ProgressDrawable(var context: Context, private val mParent: View) : Drawable(), Animatable {
    ...
    private val mCallback = object : Drawable.Callback {
        ...
        override fun scheduleDrawable(drawable: Drawable, runnable: Runnable, long: Long) {
    ...
    override fun start() {
        mAnimation?.apply {
            ...
            if (mRing.endTrim != mRing.startTrim) {
                mFinishing = true
                duration = (ANIMATION_DURATION / 2).toLong()
                mParent.startAnimation(this)
            } else {
                mRing.setColorIndex(0)
    ...
    private fun getMinProgressArc(ring: Ring): Float {
        return Math.toRadians(ring.strokeWidth / (2.0 * Math.PI * ring.centerRadius)).toFloat()
    }
    ...
    private fun updateRingColor(interpolatedTime: Float, ring: Ring) {
        if (interpolatedTime > COLOR_START_DELAY_OFFSET) {
            ...
            ring.setColor(
                evaluateColorChange(
                    (interpolatedTime - COLOR_START_DELAY_OFFSET) / (1.0f - COLOR_START_DELAY_OFFSET), ring.startingColor,
                    ring.nextColor
                )
            )
        }
        ...
        val animation = object : Animation() {
            public override fun applyTransformation(interpolatedTime: Float, t: Transformation) {
                if (mFinishing) {
                    ...
                    ring.rotation = startingRotation + 0.25f * interpolatedTime
    ...
    private class Ring(private val mCallback: Drawable.Callback) {
        ...
        private fun drawTriangle(canvas: Canvas, startAngle: Float, sweepAngle: Float, bounds: Rect) {
            if (mShowArrow) {
